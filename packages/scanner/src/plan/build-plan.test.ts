...
function value(role: string, val: string): ObservedValue {
  return {
    role,
    value: val,
    ...
    file: 'page.tsx',
    line: 1,
    // Filled in by extraction() from the containing section's path.
    jsxPath: '',
  }
}
...
function extraction(
  ...
  for (const v of sections.flatMap((s) => s.values)) {
  ...
  return {
    ...
    sections: sections.map((s) => ({
      ...
      tagName: 'div',
      snippet: '',
      values: s.values.map((v) => ({ ...v, jsxPath: s.jsxPath })),
    })),
    texts: [],
    allColors: colorSet,
    allFonts: new Set(),
