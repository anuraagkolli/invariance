// The session turn-machine (§4.4). Pure + plane-agnostic — it orchestrates the engine half
// (parseSpec → mergeDelta → diffSpecs → compile → verify) and so lives here beside merge/diff,
// imported by both planes. (resetToPublished/resetToAppDefault stay in the control plane: they
// depend on AuditStore, a storage interface.)
import { canonicalize, mergeDelta } from "./merge.js";
import { diffSpecs, type FieldDiff } from "./diff.js";
import { parseSpec, type StyleSpec, type WallFailure } from "../spec/index.js";
import { compile, type CandidateTheme } from "../compile/index.js";
import { verify, type VerifyFailure } from "../verify/index.js";
