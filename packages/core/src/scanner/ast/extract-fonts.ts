...
export function extractFonts(sourceFile: SourceFile, tw: TailwindMaps): ObservedValue[] {
  ...
  // 1. Next.js font import families. These live at the module level, not on a
  //    JSX element, so jsxPath is empty — they aggregate into the global font
  //    allowlist but never attach to a specific slot.
