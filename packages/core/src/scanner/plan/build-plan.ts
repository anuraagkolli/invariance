...
import type { InvarianceConfig, ThemeGlobals, ThemeJson } from '../../config/types'
...
function countUnassigned(
  ...
  for (const v of extraction.values) {
    // Module-level values (no jsxPath) aggregate into globals, not slots —
    // don't count them as "unassigned slot values".
    if (!v.jsxPath) continue
