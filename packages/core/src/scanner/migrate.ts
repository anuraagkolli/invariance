...
import { injectProvider, writeEnvExample } from './emit/provider-injector'
...
export async function analyze(opts: MigrateOptions): Promise<AnalyzeResult> {
  ...
  for (const page of discovered.pages) {
    ...
    // Flat list of every observed value for this page — includes the page
    // file and all same-project component files referenced by it. Slot-plan
    // attaches each value to its smallest containing slot by jsxPath.
    const flatValues: ObservedValue[] = []
    for (const values of fileObserved.values()) flatValues.push(...values)
...
export async function writeMigration(result: AnalyzeResult): Promise<void> {
  ...
  if (result.layoutFile) {
    const injection = await injectProvider(result.layoutFile, result.appRoot, result.plan.config)
    if (!injection.layoutPatched && injection.manualInstructions) {
      process.stderr.write(
        `warning: could not auto-patch ${path.relative(result.appRoot, result.layoutFile)}.\n` +
          injection.manualInstructions,
      )
    }
  }
  await writeEnvExample(result.appRoot)
}
