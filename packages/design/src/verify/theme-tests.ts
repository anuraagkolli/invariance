...
export function colorInPalette(
  ...
  for (const [key, value] of Object.entries(theme.globals?.colors ?? {}) as Array<[string, string]>) {
  ...
  for (const [slot, styles] of Object.entries(theme.slots ?? {}) as Array<[string, Record<string, string>]>) {
...
export function radiiBounded(theme: ThemeSection): TestResult {
  const violations: string[] = []
  for (const [key, value] of Object.entries(theme.globals?.radii ?? {}) as Array<[string, number]>) {
    if (value < 0 || value > 64) {
      violations.push(`${key}: ${value}`)
    }
  }
