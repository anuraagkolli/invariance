...
│       └── ...
│
│   NOTE: on `main`, apps/demo is the unintegrated React/Tailwind app (no
│         Invariance imports, no var(--inv-*)). Run the scanner against it
│         directly to test migration end-to-end. The config.yaml / theme.json /
│         providers.tsx above are generated by `--apply` (not committed on main).
```
