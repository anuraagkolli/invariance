...
| LLM | Anthropic API (Claude Sonnet) via raw fetch | Gatekeeper + Builder. No SDK. |
...
│
├── apps/demo/
│   ├── invariance.config.yaml       # Invariant definitions
│   ├── invariance.theme.initial.json # Scanner-generated initial theme
│   └── src/ ...
```
...
API: Claude Sonnet, temp 0.2, max_tokens 1024, JSON-only.
...
API: Claude Sonnet, temp 0.2, max_tokens 4096, JSON-only.
...
One-time CLI: `npx invariance scan <app-path> [--apply]`
...
Output: `invariance.config.yaml` (locked), `invariance.theme.initial.json` (original values), modified source, migration report.

Variable naming: `--inv-{slot}-{property}` (e.g., `--inv-sidebar-bg`, `--inv-header-border-color`). Short aliases: bg, text, border, font, pad, radius.
