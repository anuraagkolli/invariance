...
- `npx invariance init` (2026-06-13) is the guided onboarding wrapper around scan. It adds the "choose your invariants" step the original design left to manual unlocking: a deterministic advisor recommends a customization level per slot, the developer confirms (the LLM never picks a level — it only labels), and the confirmed choice raises the `m.slot` level + page level + unlocks colors together. It also emits the `:root` token baseline into globals.css and injects the SSR `<style>`, so a scanned app is themeable on first paint, not merely wrapped. A functional-parity test scans a clean Nebula fixture to a demo-equivalent app.
